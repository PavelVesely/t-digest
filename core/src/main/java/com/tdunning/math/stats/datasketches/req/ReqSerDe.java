...
class ReqSerDe {
  ...
  static ReqSketch heapify(final Memory mem) throws Exception {
    ...
    switch (deserFormat) {
      ...
      case RAWITEMS: {
        ...
        for (int i = 0; i < numRawItems; i++) { sk.update(buff.getDouble()); }
        return sk;
      }
      case EXACT: {
        assert preInts == 2;
        ...
        final double minValue = compactor.minVal;
        final double maxValue = compactor.maxVal;
      ...
      default: { //ESTIMATION
        ...
        final double minValue = buff.getDouble();
        final double maxValue = buff.getDouble();
  ...
  static final Compactor extractCompactor(final Buffer buff, final boolean lvl0Sorted,
    ...
    buff.getDoubleArray(arr, 0, count);
    ...
    final DoubleBuffer fltBuf = DoubleBuffer.reconstruct(arr, count, cap, delta, lvl0Sorted, hra);
  ...
  static class Compactor {
    ...
    Compactor(final ReqCompactor reqCompactor, final double minValue, final double maxValue,
        final int count) {
  ...
  static byte[] toByteArray(final ReqSketch sk) {
    ...
    switch (serDeFormat) {
      ...
      case RAWITEMS: {
        ...
        final DoubleBuffer fbuf = c0.getBuffer();
        for (int i = 0; i < numRawItems; i++) { wbuf.putDouble(fbuf.getItem(i)); }
        assert wbuf.getPosition() == bytes;
        return arr;
      }
      case EXACT: {
      ...
      default: { //Normal
        ...
        wbuf.putDouble(sk.getMinValue());
        wbuf.putDouble(sk.getMaxValue());
        for (int i = 0; i < numCompactors; i++) {
  ...
  static int getSerBytes(final ReqSketch sk, final SerDeFormat serDeFormat) {
    switch (serDeFormat) {
      ...
      case RAWITEMS: {
        return sk.getCompactors().get(0).getBuffer().getCount() * Double.BYTES + 8;
      }
      case EXACT: {
